...
function syncButton(){
 const old=document.getElementById('generateSelectedPendingTablePdf');
 if(old){old.id='generateSelectedPendingTablePdfClaro';old.textContent='Gerar planilha selecionada'}
 const btn=document.getElementById('generateSelectedPendingTablePdfClaro');
...
async function generate(){
 ...
 try{
  ...
  const drawHeader=()=>{
   ...
   for(const [name,w] of cols){doc.setFillColor(232,242,252);doc.setDrawColor(157,197,234);doc.setLineWidth(.2);doc.rect(x,19,w,10,'FD');doc.setTextColor(23,50,79);doc.text(name,x+w/2,25,{align:'center',maxWidth:w-2});x+=w}
   return 29;
  };
  let y=drawHeader();
  for(const i of items){
   ...
   for(let n=0;n<cols.length;n++){const w=cols[n][1];if(accepted)doc.setFillColor(239,250,243);else doc.setFillColor(255,248,239);doc.setDrawColor(185,212,237);doc.setLineWidth(.2);doc.rect(x,y,w,h,'FD');doc.setTextColor(25,43,68);doc.text(lines[n],x+1.2,y+4.2,{maxWidth:w-2.4});x+=w}
   y+=h;
  }
