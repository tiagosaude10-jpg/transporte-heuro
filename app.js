...
function seedFirstAdmin(){
  ...
  users.push({id:'admin-inicial',fullName:'Tiago Pereira de Albuquerque',cpf:'',email:'',phone:'',username:'Tiago',password:'1234',profile:'administrador',status:'ativo',createdAt:new Date().toISOString(),approvedAt:new Date().toISOString(),approvedBy:'Configuração inicial'});
  saveUsers(users);
}
seedFirstAdmin();

$('welcomeEnter').addEventListener('click',()=>showScreen(loginScreen));
$('welcomeChangeUser').addEventListener('click',()=>{localStorage.removeItem('heuroRememberedUser');sessionStorage.removeItem('heuroSession');$('userName').value='';$('password').value='';showScreen(loginScreen);setTimeout(()=>$('userName').focus(),50);});
...
$('cpf').addEventListener('input',e=>{let v=cleanDigits(e.target.value).slice(0,11);v=v.replace(/(\d{3})(\d)/,'$1.$2').replace(/(\d{3})(\d)/,'$1.$2').replace(/(\d{3})(\d{1,2})$/,'$1-$2');e.target.value=v;});

$('registerForm').addEventListener('submit',e=>{
  e.preventDefault();
  const selected=document.querySelector('input[name="registerProfile"]:checked');
  const cpf=cleanDigits($('cpf').value), email=normalize($('email').value), username=$('newUserName').value.trim(), password=$('newPassword').value, users=getUsers();
  ...
  users.push({id:crypto.randomUUID?crypto.randomUUID():String(Date.now()),fullName:$('fullName').value.trim(),cpf,email,phone:$('phone').value.trim(),username,password,profile:selected.value,status:'aguardando',createdAt:new Date().toISOString(),approvedAt:null,approvedBy:null});
  saveUsers(users);$('registerForm').reset();setMessage('registerMessage','Cadastro enviado com sucesso. Aguarde a autorização de um administrador.','success');
...
$('loginForm').addEventListener('submit',e=>{
  ...
  const username=normalize($('userName').value),password=$('password').value,user=getUsers().find(u=>normalize(u.username)===username);
  ...
  currentProfile=user.profile;const activeSession={id:user.id,name:user.fullName||user.username,username:user.username,profile:user.profile};
...
function enterHome(s){currentProfile=s.profile;$('welcomeName').textContent=`Olá, ${s.name}`;$('welcomeRole').textContent=profileLabels[s.profile]||s.profile;$('usersCard').classList.toggle('hidden',s.profile!=='administrador');$('newRequestCard').classList.toggle('hidden',s.profile==='transporte');$('settingsCard').classList.toggle('hidden',s.profile!=='administrador');showScreen(homeScreen);showDashboard();}
...
function updatePendingBadge(){const count=getUsers().filter(u=>u.status==='aguardando').length;$('pendingBadge').textContent=count;$('pendingBadge').classList.toggle('hidden',!count);}
function renderUsers(){
  if(currentProfile!=='administrador'){showDashboard();return;}
  const users=getUsers().filter(u=>u.id!=='admin-inicial').sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt)),box=$('userApprovalList');
  if(!users.length){box.innerHTML='<div class="card empty">Nenhum cadastro de usuário recebido neste aparelho.</div>';return;}
  box.innerHTML=users.map(u=>`<article class="card user-card"><div class="user-card-head"><div><span class="status ${u.status==='ativo'?'done':u.status==='recusado'?'rejected':''}">${statusLabel(u.status)}</span><h4>${escapeHtml(u.fullName)}</h4><small>@${escapeHtml(u.username)} · ${escapeHtml(profileLabels[u.profile])}</small></div></div><div class="user-data"><span><b>CPF</b>${escapeHtml(formatCpf(u.cpf))}</span><span><b>E-mail</b>${escapeHtml(u.email)}</span><span><b>Telefone</b>${escapeHtml(u.phone)}</span></div><label class="compact-label">Perfil autorizado<select data-profile="${u.id}"><option value="solicitante" ${u.profile==='solicitante'?'selected':''}>Solicitante de transporte</option><option value="transporte" ${u.profile==='transporte'?'selected':''}>Executante de transporte</option><option value="administrador" ${u.profile==='administrador'?'selected':''}>Administrador</option></select></label><div class="approval-actions"><button class="success-button" data-approve="${u.id}">Autorizar</button><button class="danger-button" data-reject="${u.id}">Recusar</button>${u.status==='ativo'?`<button class="secondary-button" data-block="${u.id}">Bloquear</button>`:''}</div></article>`).join('');
  box.querySelectorAll('[data-approve]').forEach(b=>b.onclick=()=>changeUserStatus(b.dataset.approve,'ativo'));box.querySelectorAll('[data-reject]').forEach(b=>b.onclick=()=>changeUserStatus(b.dataset.reject,'recusado'));box.querySelectorAll('[data-block]').forEach(b=>b.onclick=()=>changeUserStatus(b.dataset.block,'bloqueado'));
}
function changeUserStatus(id,status){const data=getUsers(),i=data.findIndex(u=>u.id===id);if(i<0)return;const profileSelect=document.querySelector(`[data-profile="${id}"]`);if(profileSelect)data[i].profile=profileSelect.value;data[i].status=status;data[i].approvedAt=status==='ativo'?new Date().toISOString():data[i].approvedAt;data[i].approvedBy=session()?.name||'Administrador';saveUsers(data);renderUsers();updatePendingBadge();}
...
function updateOriginLocation(){
  ...
  $('boxLocation').classList.toggle('hidden',!usesBox);
  $('wardLocation').classList.toggle('hidden',usesBox);
  $('boxNumber').required=usesBox;$('ward').required=!usesBox;$('bed').required=!usesBox;
  if(usesBox){$('ward').value='';$('bed').value='';}else{$('boxNumber').value='';}
}
$('originSector').addEventListener('change',updateOriginLocation);
updateOriginLocation();

$('requestForm').addEventListener('submit',e=>{
  e.preventDefault();const file=$('attachment').files[0],s=session(),usesBox=['Sala Vermelha','UTI'].includes($('originSector').value);
  const item={id:crypto.randomUUID?crypto.randomUUID():String(Date.now()),protocol:protocol(),status:'Solicitado',createdAt:new Date().toISOString(),requester:s?.name||'Usuário',requesterId:s?.id||'',patient:$('patient').value.trim(),birthDate:$('birthDate').value,originSector:$('originSector').value,boxNumber:usesBox?$('boxNumber').value.trim():'',ward:usesBox?'':$('ward').value.trim(),bed:usesBox?'':$('bed').value.trim(),destination:$('destination').value.trim(),transportDate:$('transportDate').value,transportTime:$('transportTime').value,ambulanceType:$('ambulanceType').value,priority:$('priority').value,oxygen:$('oxygen').value,contact:$('contact').value.trim(),notes:$('notes').value.trim(),attachmentName:file?file.name:'Nenhum documento informado'};
  const data=requests();data.unshift(item);saveRequests(data);$('requestForm').reset();updateOriginLocation();alert(`Solicitação ${item.protocol} salva.`);openDetail(item.id);
});

function visibleRequests(){const s=session(),data=requests();return s?.profile==='solicitante'?data.filter(r=>r.requesterId===s.id||(!r.requesterId&&r.requester===s.name)):data;}
function renderList(){const data=visibleRequests(),box=$('requestList');if(!data.length){box.innerHTML='<div class="card empty">Nenhuma solicitação disponível para este usuário.</div>';return;}box.innerHTML=data.map(r=>`<button class="request-card" data-id="${r.id}"><span class="status ${r.status==='Concluído'?'done':''}">${r.status}</span><strong>${escapeHtml(r.patient)}</strong><small>${escapeHtml(r.protocol)}</small><small>${formatDate(r.transportDate)} às ${escapeHtml(r.transportTime)}</small><span>${escapeHtml(r.originSector||r.origin||'Origem não informada')} → ${escapeHtml(r.destination)}</span></button>`).join('');box.querySelectorAll('[data-id]').forEach(b=>b.addEventListener('click',()=>openDetail(b.dataset.id)));}
function locationText(r){return r.boxNumber?`Box: ${r.boxNumber}`:`Enfermaria: ${r.ward||'Não informada'}\nLeito: ${r.bed||'Não informado'}`;}
