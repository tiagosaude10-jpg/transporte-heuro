...
async function toggleConclusion(id){
 ...
 if(completing){
  ...
  item.history=[...(item.history||[]),history('Concluído')];
 }else{
  ...
  item.status=prior==='accepted'?'Aceito':'Solicitado';
  item.completedAt='';
  item.history=[...(item.history||[]),history(item.status)];
  visibleCompleted.delete(id);
  if(prior==='accepted')acceptedThisView.add(id);
 }
 save(data);render();return true
}
async function finalizeCompleted(){
 if(!visibleCompleted.size)return;
 const data=read();
 for(const[id]of [...visibleCompleted]){
  const item=data.find(x=>x.id===id);
  if(item?.status==='Concluído'){
   await deleteAttachment(id);
   item.attachmentDeletedAt=new Date().toISOString();
   item.attachmentName='Imagem excluída após conclusão';
   item.attachmentAvailable=false;
  }
 }
 save(data);visibleCompleted.clear();acceptedThisView.clear()
}
document.addEventListener('click',e=>{
 const pending=e.target.closest('#openPendingTeam');
 if(pending){pendingViewActive=true;acceptedThisView=new Set();visibleCompleted=new Map();setTimeout(render,80);return}
 const a=e.target.closest('.pending-accept');
 if(a){e.preventDefault();e.stopImmediatePropagation();toggleAccept(a.dataset.id);return}
 const c=e.target.closest('.pending-conclude');
 if(c){e.preventDefault();e.stopImmediatePropagation();toggleConclusion(c.dataset.id);return}
...
window.addEventListener('click',async e=>{
 const detail=$('transportDetailNew');if(!detail?.classList.contains('active'))return;
 const action=e.target.closest('[data-status]');if(!action||!detail.contains(action))return;
 const id=action.dataset.id;const status=action.dataset.status;
 if(!id||!['Aceito','Concluído'].includes(status))return;
 e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();
 const changed=status==='Aceito'?await toggleAccept(id):await toggleConclusion(id);
 if(changed){showTeam();render()}
},true);
})();