(() => {
  ...
  /*
   * O painel antigo é substituído pela imagem oficial. Mantemos somente os IDs
   * que o app.js realmente utiliza dentro do dashboard. Não repetimos os IDs
   * welcomeName, welcomeRole ou logoutButton, que já existem no cabeçalho.
   */
  ...
  dashboard.innerHTML = `
    <div class="command-image-frame" aria-label="Página de comando do Transporte HEURO">
      <img class="command-image" src="${COMMAND_IMAGE}?v=20260731-4" alt="Página de comando do aplicativo Transporte HEURO" decoding="async" fetchpriority="high" />
  ...
  const openView = (viewId) => {
    ...
    else if (typeof showView === 'function') showView(viewId);
  };
  ...
  document.getElementById('homeNavCard')?.addEventListener('click', openDashboard);
  ...
  style.textContent = `
    ...
    .command-hotspot{position:absolute;z-index:4;margin:0;padding:0;border:0;border-radius:14px;background:transparent;appearance:none;-webkit-appearance:none;touch-action:manipulation}
    .command-hotspot:focus-visible{outline:3px solid rgba(11,95,165,.55);outline-offset:-3px}
    .command-system-control{position:absolute;width:1px;height:1px;overflow:hidden;clip:rect(0 0 0 0);white-space:nowrap}
    ...
    #welcomeScreen.welcome-screen.active{position:fixed;inset:0;width:100%;height:100dvh;min-height:0;padding:0!important;overflow:hidden!important;display:flex;align-items:center;justify-content:center;background:#fff;z-index:9999}
    #welcomeScreen .welcome-frame{position:relative;width:min(100%,620px);height:100%;min-height:0;overflow:hidden;background:#fff}
    #welcomeScreen .welcome-image{display:block;width:100%;height:100%;object-fit:contain;object-position:center center;background:#fff}
    html:has(#welcomeScreen.active),body:has(#welcomeScreen.active){height:100%;overflow:hidden!important;overscroll-behavior:none}
  `;
