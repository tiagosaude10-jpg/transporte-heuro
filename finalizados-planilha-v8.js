...
function render(date=today()){
 ...
 const heads=['Selecionar','Protocolo','Solicitado','Aceite','Conclusão','Paciente','Origem','Destino','Horário agendado','Status','Solicitante','Executante','Ambulância','Prioridade','PDF'];
 ...
 <div style="overflow-x:auto;background:#fff;border-radius:16px;border:1px solid #dbe5ef"><table style="width:100%;border-collapse:collapse;min-width:2280px"><thead><tr style="background:#f3eef9">${heads.map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${data.map(row).join('')||'<tr><td colspan="15" style="padding:22px;text-align:center">Nenhum transporte finalizado nesta data.</td></tr>'}</tbody></table></div>
