...
.pending-sheet-tools{flex:0 0 auto;display:grid;grid-template-columns:1fr 1fr;gap:9px;align-items:center;padding:10px;background:#fff;border:1px solid #dbe5ef;border-radius:14px;box-shadow:0 5px 14px rgba(20,40,80,.06)}
.pending-sheet-tool{width:100%;min-height:42px;border:0;border-radius:10px;padding:0 13px;font-weight:900;box-sizing:border-box}.pending-sheet-tool.secondary{background:#e8eef7;color:#243b64}.pending-sheet-tool.primary{background:#0b5fa5;color:#fff}.pending-sheet-tool.table-pdf{background:#5b3f82;color:#fff}.pending-sheet-tool:disabled{opacity:.45}.pending-selected-count{grid-column:1/-1;color:#243b64;font-size:13px}.pending-color-legend{grid-column:1/-1;border-top:1px solid #d8e5f1;border-bottom:1px solid #d8e5f1;padding:9px 0;display:flex;flex-wrap:wrap;gap:10px 16px;color:#243b64;font-size:12px;font-weight:800}.pending-color-legend span{display:inline-flex;align-items:center;gap:7px}.pending-color-swatch{width:17px;height:17px;border-radius:5px;box-shadow:inset 0 0 0 1px rgba(0,0,0,.08)}.pending-color-swatch.pending{background:#ef9ca2}.pending-color-swatch.accepted{background:#f4b36f}
...
.pending-sheet-table{border-collapse:separate;border-spacing:0;min-width:2080px;width:100%}.pending-sheet-table th,.pending-sheet-table td{padding:11px;text-align:center;white-space:nowrap;border-bottom:1px solid #b9d4ed}.pending-sheet-table th{position:sticky;top:0;z-index:4;background:#eaf3fb;color:#17324f;font-weight:900}.pending-sheet-table th:nth-child(2),.pending-sheet-table td:nth-child(2){text-align:left}.pending-sheet-table tr.pending-row td{background:#f9c9cd}.pending-sheet-table tr.accepted-row td{background:#f8d3aa}.pending-sheet-table tr.highlight td{outline:3px solid #0b6fc1;outline-offset:-3px}.status-pending,.status-accepted{display:inline-block;padding:6px 9px;border-radius:999px;color:#fff;font-weight:900}.status-pending{background:#c62828}.status-accepted{background:#e97800}.ps-pdf{border:0;border-radius:10px;padding:9px 12px;background:#243b64;color:#fff;font-weight:900;white-space:nowrap}.pending-select{width:22px;height:22px;accent-color:#0b5fa5}
@media(max-width:520px){.pending-sheet-tools{grid-template-columns:1fr 1fr}.pending-sheet-tool.primary,.pending-sheet-tool.table-pdf{grid-column:1/-1}.pending-color-legend{display:grid;gap:7px}}
`;document.head.appendChild(st)}return s
}
function row(i){
 const accepted=i.status==='Aceito';
 const cls=accepted?'accepted-row':'pending-row';
 const status=accepted?'Aceito / Em andamento':'Solicitado';
 return `<tr data-id="${esc(i.id)}" class="${cls}${String(i.id)===String(selectedId)?' highlight':''}"><td><input class="pending-select" data-select-id="${esc(i.id)}" type="checkbox" ${selected.has(String(i.id))?'checked':''} aria-label="Selecionar ${esc(i.patient||'transporte')}"></td><td><strong>${esc(i.patient||'Paciente')}</strong></td><td>${esc(i.protocol||'—')}</td><td>${esc(dt(i.createdAt))}</td><td>${esc(dt(i.acceptedAt))}</td><td>${esc(origin(i))}</td><td>${esc(i.destination||'Não informado')}</td><td>${esc(scheduled(i))}</td><td><span class="${accepted?'status-accepted':'status-pending'}">${status}</span></td><td>${esc(i.requester||'Não informado')}</td><td>${esc(i.executor||'—')}</td><td><button class="ps-pdf" data-pdf-id="${esc(i.id)}">Abrir PDF</button></td></tr>`
}
...
function render(){
 ...
 const heads=['Selecionar','Paciente','Protocolo','Solicitado em','Aceite em','Origem','Destino','Horário agendado','Status','Solicitado por','Aceito por','PDF'];
 $('pendingSheetContent').innerHTML=`<div class="pending-sheet-tools"><button id="selectAllPending" class="pending-sheet-tool secondary" type="button">Marcar todos</button><button id="clearPendingSelection" class="pending-sheet-tool secondary" type="button">Limpar seleção</button><div class="pending-color-legend"><span><i class="pending-color-swatch pending"></i>Vermelho: Solicitado / aguardando aceite</span><span><i class="pending-color-swatch accepted"></i>Laranja: Aceito / em andamento</span></div><button id="generateSelectedPendingPdf" class="pending-sheet-tool primary" type="button" ${selected.size?'':'disabled'}>Gerar PDFs dos transportes</button><button id="generateSelectedPendingTablePdf" class="pending-sheet-tool table-pdf" type="button" ${selected.size?'':'disabled'}>Gerar planilha selecionada</button><strong id="pendingSelectedCount" class="pending-selected-count">${selected.size} transporte(s) selecionado(s)</strong></div><div class="pending-sheet-wrap"><table class="pending-sheet-table"><thead><tr>${heads.map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>${data.map(row).join('')||'<tr><td colspan="12" style="padding:24px">Nenhuma pendência encontrada.</td></tr>'}</tbody></table></div>`;
...
async function tablePdf(){
 ...
 try{
  ...
  const columns=[['Paciente',30],['Protocolo',22],['Solicitado',27],['Aceite',27],['Origem',38],['Destino',32],['Agendado',28],['Status',27],['Solicitado por',28],['Aceito por',28]];
  const margin=4,pageW=doc.internal.pageSize.getWidth(),pageH=doc.internal.pageSize.getHeight(),usable=pageW-margin*2,total=columns.reduce((s,c)=>s+c[1],0),widths=columns.map(c=>c[1]*usable/total);
  const drawHeader=()=>{doc.setFont('helvetica','bold');doc.setFontSize(13);doc.text('TRANSPORTES HEURO — PLANILHA DE PENDÊNCIAS SELECIONADAS',pageW/2,10,{align:'center'});doc.setFontSize(7);doc.setFillColor(225,238,250);let x=margin;columns.forEach(([name],n)=>{doc.rect(x,15,widths[n],9,'F');doc.text(name,x+widths[n]/2,20.5,{align:'center',maxWidth:widths[n]-2});x+=widths[n]});return 24};
  let y=drawHeader();doc.setFont('helvetica','normal');doc.setFontSize(6.2);
  for(const i of items){const values=[i.patient||'Paciente',i.protocol||'—',dt(i.createdAt),dt(i.acceptedAt),origin(i),i.destination||'Não informado',scheduled(i),i.status==='Aceito'?'Aceito / Em andamento':'Solicitado',i.requester||'Não informado',i.executor||'—'];const lines=values.map((v,n)=>doc.splitTextToSize(String(v),widths[n]-2));const h=Math.max(9,...lines.map(l=>l.length*3+3));if(y+h>pageH-6){doc.addPage();y=drawHeader();doc.setFont('helvetica','normal');doc.setFontSize(6.2)}let x=margin;const accepted=i.status==='Aceito';doc.setFillColor(...(accepted?[248,211,170]:[249,201,205]));columns.forEach(([,w],n)=>{doc.rect(x,y,widths[n],h,'F');doc.setDrawColor(185,212,237);doc.rect(x,y,widths[n],h);doc.setTextColor(23,50,79);doc.text(lines[n],x+1,y+4);x+=widths[n]});y+=h}
